...
#define BEACON_DIST_us 128000000
...
static void s2e_bcntimeout (tmr_t* tmr) {
    ...
    if( gpstime == 0 || !(latlon_ok = sys_getLatLon(&lat, &lon)) ) {
        ...
        LOG(MOD_S2E|INFO, "Beaconing suspended - insufficient GPS data: %s %s",
            gpstime==0?"time":"", latlon_ok?"position":"");
        rt_setTimer(tmr, now + rt_seconds(10));
        return;
    }
    
    // Next beacon TX time is on upcoming multipl of 128s GPS time which is at least 1s ahead
    sL_t gpstxtime = gpstime + BEACON_DIST_us + rt_seconds(1);
    gpstxtime -= gpstxtime % (BEACON_DIST_us-1);
    sL_t ahead = gpstxtime - gpstime;
    if( ahead > rt_seconds(1) ) {
        // Sleep some more to get closer to TX time
    ...
    uint32_t epoch = gpstxtime/BEACON_DIST_us;
  ...
  nextbcn:
    // Sleep until next beacon is 1s ahead
    rt_setTimer(tmr, now + ahead + BEACON_DIST_us);
}
...
static int handle_router_config (s2ctx_t* s2ctx, ujdec_t* D) {
    ...
    if( sx1301conf.buf == NULL ) {
        LOG(MOD_S2E|ERROR, "No 'sx1301_conf' in 'router_config' message");
        return 0;
    }
    int chdefs = chslots;
